...
struct inode* mfs_get_inode(struct super_block* sb, const struct inode* dir, int mode);
...
static ssize_t mfs_metrics_read(struct file* file, char __user* buf, size_t len, loff_t* offset);
static ssize_t mfs_metrics_write(
...
static const struct file_operations mfs_metrics_operations
    = { .read = mfs_metrics_read, .write = mfs_metrics_write, .llseek = generic_file_llseek };
...
static ssize_t mfs_metrics_read(struct file* file, char __user* buf, size_t len, loff_t* offset)
{
    ...
    u64 bytes_written = atomic64_read(&total_bytes_written);
    u64 bytes_read = atomic64_read(&total_bytes_read);
    u64 write_ops = atomic64_read(&total_write_ops);
    u64 read_ops = atomic64_read(&total_read_ops);
    u64 files_created = atomic64_read(&total_files_created);
    u64 files_deleted = atomic64_read(&total_files_deleted);
    u64 files_current = files_created - files_deleted;
    u64 dirs_created = atomic64_read(&total_dirs_created);
    u64 dirs_deleted = atomic64_read(&total_dirs_deleted);
    u64 dirs_current = dirs_created - dirs_deleted;

    int metrics_len = scnprintf(metrics_buf, sizeof(metrics_buf),
        "bytes written: %lld\n"
        "bytes read: %lld\n"
        ...
        "files created: %lld\n"
        "files deleted: %lld\n"
        "current number of files: %lld\n"
        "dirs created: %lld\n"
        "dirs deleted: %lld\n"
        "current number of dirs: %lld\n",
        bytes_written, bytes_read, write_ops, read_ops, files_created, files_deleted, files_current,
        dirs_created, dirs_deleted, dirs_current);
...
static int mfs_fill_super(struct super_block* sb, void* data, int silent)
{
    ...
    root_inode = mfs_get_inode(sb, NULL, S_IFDIR | S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
    ...
    // Set custom read/write operations
    metrics_inode->i_fop = &mfs_metrics_operations;
    // Set metric inode's immutable flag so it can't be renamed, moved or deleted
    metrics_inode->i_flags |= S_IMMUTABLE;
...
static struct dentry* mfs_mount(
...
{
    atomic64_set(&total_bytes_written, 0);
    atomic64_set(&total_bytes_read, 0);
    atomic64_set(&total_read_ops, 0);
    atomic64_set(&total_write_ops, 0);
    atomic64_set(&total_inodes_created, 0);
    atomic64_set(&total_files_created, 0);
    atomic64_set(&total_files_deleted, 0);
    atomic64_set(&total_dirs_created, 0);
    atomic64_set(&total_dirs_deleted, 0);
}
