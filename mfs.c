...
#include <linux/magic.h> // Contains magic numbers for various filesystems, e.g., RAMFS_MAGIC
...
/**
 * ramfs_get_inode - Creates a new inode for a file or directory.
...
static int ramfs_parse_param(struct fs_context *fc,
   ...
   * the specifications in `ramfs_fs_parameters`.
   */
...
static int ramfs_fill_super(struct super_block *sb, struct fs_context *fc) {
  ...
  sb->s_magic = RAMFS_MAGIC;         // Filesystem's unique identifier.
  sb->s_op = &ramfs_ops;             // Assign the superblock operations.
...
/**
 * ramfs_init_fs_context - Entry point for starting a mount operation.
...
static struct file_system_type ramfs_fs_type = {
    ...
    .parameters = ramfs_fs_parameters, // Describes mount parameters.
    .kill_sb = ramfs_kill_sb,          // Function to call on unmount.
